...
async def delete_all_items(partition_key: str):
    ...
    try:
        # TODO: Query all items with partition key and delete them
        # Hint: Use "SELECT c.id FROM c WHERE c.pk = @pk" with enable_cross_partition_query=True
        # Loop through items and call container.delete_item(item["id"], partition_key)
...
async def upsert_snippet(doc_id, text, pk="ecommerce"):
    ...
    try:
        # TODO: Generate embeddings and upsert document to Cosmos DB
        # Hint: Call embed_texts([text]) to get embeddings list, take first element
        # Create dict with keys: id, pk, text, embedding
        # Call container.upsert_item() with the document dict
        print(f"{doc_id} upserted with Semantic Kernel embeddings.")
    except Exception as e:
        print(f"Failed to upsert {doc_id}: {e}")

async def main():
    """Main function to upsert ecommerce data"""

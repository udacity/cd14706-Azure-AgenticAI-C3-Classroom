...
async def retrieve_with_text_search(query: str, k: int = 5, partition_key: str = None) -> List[Dict[str, Any]]:
    ...
    - Works best with simple keyword queries (e.g., "Lakers", "Warriors")
...
async def retrieve_with_hybrid_search(query: str, k: int = 5, partition_key: str = None) -> List[Dict[str, Any]]:
    ...
    This function combines the strengths of both search methods:
    - Vector search: Semantic similarity, works for all query types
    - Text search: Keyword matching, fast for simple queries
    - RRF reranking: Merges results using Reciprocal Rank Fusion

    Even if text search returns 0 results, vector search ensures relevant
    documents are retrieved. The hybrid approach improves result quality when
